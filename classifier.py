...
# Keep scikit-learn / BLAS work bounded on CPU-only laptops.
...
def _build():
    return Pipeline([
        ...
        ("clf", LogisticRegression(max_iter=300, C=4.0, random_state=42, solver="liblinear")),
    ])
...
def train_with_feedback(min_samples=20, save=True):
    ...
    # 3. Safety gate
    # --------------------------------------------------------------
    # Do not reject an adaptive candidate for tiny holdout-set noise.
    # A 3 percentage-point guardrail prevents an inconsequential fluctuation
    # from making every analyst feedback action look like a failure.
    validation_tolerance = 0.03
    if candidate_accuracy + validation_tolerance < baseline_accuracy:
        ...
        return None, {
            ...
            "message": (
                "Adaptive model rejected because validation accuracy regressed "
                f"by more than {validation_tolerance:.0%}."
            ),
        }
